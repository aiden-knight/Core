...
void init_memory_tracking() {
	...
	g_core_ptr->memory_tracking->flags = 0U;
...
void* track_allocation_internal(void* allocation, char* function, u32 line_number) {
	...
	if (is_memeory_tracking_flag_active(MTF_IGNORE)) return allocation;
...
static void recursively_track_frees(AllocatorTrackingData* allocator_data, void* allocation) {
	...
	if (allocator_data->allocations[index].is_allocator)
	{
		...
		// Note(Tom): see note in start tracknig allocator about assuming an allocator is only made up of one single allocation. May not be true
...
void track_free_internal(void* free) {
	if (is_memeory_tracking_flag_active(MTF_IGNORE)) return;
